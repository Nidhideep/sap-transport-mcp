...
import { mapImportQueueEntry, type ImportQueueEntry } from "../lib/transport-mapper.js";
...
export const importQueueTool = {
  ...
  async handler(rawInput: unknown): Promise<ImportQueueResult> {
    ...
    const raw = await adtGet<unknown>(
      system,
      `/sap/bc/adt/cts/transports/importqueue/${targetSid}`,
      { "$format": "json" }
    );

    const records = Array.isArray(raw) ? raw : [];
    const transports = records.map((r) =>
      mapImportQueueEntry(r as Record<string, unknown>)
    );
